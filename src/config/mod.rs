...
//!
//! All configuration files live under `~/.config/skap/` and are lazily
...
/// Returns the directory holding all skap configuration files
/// (`~/.config/skap` on Linux). Creates it if missing.
pub fn config_dir() -> Result<PathBuf> {
    let base = dirs::config_dir().context("could not determine user config directory")?;
    let dir = base.join("skap");
