//! `skap add` – add a feature (git, docker, ci, ...) to an existing project.
...
use crate::core::{git as skap_git, github, gitlab, ports as port_core};
...
fn add_git(dir: &Path, entry: &mut crate::config::registry::ProjectEntry) -> Result<()> {
    ...
    skap_git::init_with_initial_commit(dir, "Initial commit (skap)")?;
    entry.git = true;
