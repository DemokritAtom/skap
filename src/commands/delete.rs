...
pub async fn run(args: DeleteArgs) -> Result<()> {
    ...
    if !args.keep_files && dir.exists() {
        std::fs::remove_dir_all(&dir).with_context(|| format!("Ordner konnte nicht gelöscht werden: {}", dir.display()))?;
        output::success(&format!("Ordner {} gelöscht", dir.display()));
    }
    Ok(())
}

fn remove_registry_only(name: String, reg: &mut Registry, port_reg: &mut PortRegistry) -> Result<()> {
    reg.remove(&name);
    port_reg.release_project(&name);
    reg.save()?;
    port_reg.save()?;
